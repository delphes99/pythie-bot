...
internal class UserCacheTest {
    ...
    internal fun `get user infos when not cached`() {
        val emptyCache = UserCache(
            ...
            clock = clock,
        ) { _, now -> UserInfos(USER_NAME, now, BroadcasterType.AFFILIATE, 50) }

        val userInfos = runBlocking {
            emptyCache.getUser(USER)
        }

        assertThat(userInfos).isEqualTo(UserInfos(USER_NAME, NOW, BroadcasterType.AFFILIATE, 50))
    }

    @Test
    internal fun `cached value`() {
        val cache = UserCache(
            retrieveUser = { _, now -> UserInfos(USER_NAME, now, BroadcasterType.AFFILIATE, 50) },
            expirationDuration = Duration.ofMinutes(10),
            clock = clock,
            USER to UserInfos(USER_NAME, CACHED_TIME, BroadcasterType.AFFILIATE, 50),
        )
        ...
        assertThat(userInfos).isEqualTo(UserInfos(USER_NAME, CACHED_TIME, BroadcasterType.AFFILIATE, 50))
    }
    ...
    internal fun `retrieve fresh infos if cached values is too old`() {
        val cache = UserCache(
            ...
            expirationDuration = Duration.ofMinutes(10),
            clock = clock,
            USER to UserInfos(USER_NAME, CACHED_TIME_EXPIRED, BroadcasterType.AFFILIATE, 50),
        )
