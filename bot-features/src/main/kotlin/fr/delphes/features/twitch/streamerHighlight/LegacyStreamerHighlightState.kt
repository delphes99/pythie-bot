...
@Serializable
data class LegacyStreamerHighlightState(
    ...
    fun highlight(user: UserName, highlightDate: LocalDateTime): LegacyStreamerHighlightState {
        val newState = this.streamerHighlighted + (user.normalizeName to highlightDate)
        return LegacyStreamerHighlightState(
            newState
        )
    }
}